...
public class ThisYearFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // TODO Auto-generated method stub
        if (null == rootView) {
            rootView = inflater.inflate(R.layout.fragment_thisyear, container,
                    false);
            mChart = (LineChartView) rootView.findViewById(R.id.chart_right);
            mListView = (ListView) rootView.findViewById(R.id.lv_main_right);
    ...
    private void initView(View rootView) {
        ...
        //根据目标年份获取参数
        setYearCostData(days);
        setYearInComeData(days);
        //chart
    ...
    protected void initYearCostBean() {
        ...
        for (CostBean costBean : yearsCostList) {
            //1支出0收入
            if (costBean.colorType == 1 && costBean.getCostDate().substring(0, 4).equals(days)) {
                //查询年
                total_expend_money = total_expend_money + Double.parseDouble(costBean.getCostMoney());
            }
            if (costBean.colorType == 0 && costBean.getCostDate().substring(0, 4).equals(days)) {
                //查询年
